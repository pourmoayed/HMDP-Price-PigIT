...
//' @export
// [[Rcpp::export]]
SEXP BuildHMDP2Deterministic(const CharacterVector filePrefix, const List param, const List paramDLMP, const List paramDLMPi, const List paramDLMF
                   ,const List paramPolicy) {
   string prefix = as<string>(filePrefix);
   HMDP2 Model(prefix, param, paramDLMP, paramDLMPi, paramDLMF, paramPolicy);
   //Rcout << "Total number of states: " << Model.countStatesHMDP() << endl;
   return( Model.BuildHMDPDeterministic() );
   //return(wrap(0));
}
